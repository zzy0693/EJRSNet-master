...
# from ppdet.modeling.common import SpatialAttention
...
class TaskDecomposition(nn.Layer):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2D(self.in_channels, self.in_channels // 2, 7, stride=1, padding=9,
                               groups=self.in_channels // 2, dilation=3)
        self.conv2 = nn.Conv2D(self.in_channels, self.in_channels // 2, 7, stride=1, padding=9,
                               groups=self.in_channels // 2, dilation=3)

    def _init_weights(self):
        normal_(self.la_conv1.weight, std=0.001)
        normal_(self.la_conv2.weight, std=0.001)
        normal_(self.conv1.weight, std=0.001)
        normal_(self.conv2.weight, std=0.001)
    ...
    def forward(self, feat):
        ...
        h = feat_shape[2:3]
        w = feat_shape[3:4]
        cls_feat = self.conv1(feat)
        reg_feat = self.conv2(feat)
        # attn = paddle.concat([cls_feat, reg_feat], axis=1)
        attn = cls_feat + reg_feat
        # key -> [b, 1, H, W] -> [b, 1, H*W] ->  [b, H*W, 1]
        attn0 = F.adaptive_avg_pool2d(attn, (1, 1))
        attn1 = F.adaptive_max_pool2d(attn, (1, 1))
        attn = attn1 + attn0

        weight = F.relu(self.la_conv1(attn))
        weight = F.sigmoid(self.la_conv2(weight)).unsqueeze(-1)
        cls_feat = paddle.reshape(
            cls_feat, [b, self.stacked_convs // 2, self.feat_channels, h, w]) * weight
        reg_feat = paddle.reshape(
            reg_feat, [b, self.stacked_convs // 2 , self.feat_channels, h, w]) * weight

        cls_feat = self.reduction_conv(cls_feat.flatten(1, 2))
        cls_feat = F.relu(cls_feat)
        reg_feat = self.reduction_conv(reg_feat.flatten(1, 2))
...
class TOODHead(nn.Layer):
    ...
    def forward(self, feats):
        ...
        for feat, scale_reg, anchor_centers, stride in zip(
            ...
            cls_feat, reg_feat = self.decomp(feat)
